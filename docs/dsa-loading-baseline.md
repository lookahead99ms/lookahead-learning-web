...
npm run profile:browser -- \
  ...
  --cycles 30 \
  --budgets ../lookahead-learning-content/docs/evidence/DLV-205-performance-budget-proposal.json \
...
Budget files must declare either `proposed-not-approved` or `approved`. Proposed
numbers are compared and reported but never enforced. Activate CI or release
failure only after explicit user approval, retain the measurement definition with
the threshold, and rerun against the release candidate. Do not relax a limit just
to make a build pass; record the decision and trade-off.
